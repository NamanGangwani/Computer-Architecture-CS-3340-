...
final:	.asciiz "Final Output"	# For visual representation in the output
	...
	li $a2, 2000000		# Reads 2000000 characters maximum
	syscall			# Reads file based on system call code
	
	addi $t5, $t5, 0	# Final result = 0
	addi $t6, $t6, 1	# Holds the final result of multiplication of two integers
...
readLine:
	jal readNum		# Read the number on the current line
  	...
  	move $a0, $t7		# Puts the read integer into the register for the first argument
  	syscall			# Prints the integer based on system call code
  	
	mul $t6, $t6, $t7	# Multiplies the read integer by the current result
	beqz $t2, addThenExit	# If it has reached the end of the file, stop reading the file
...
printResult:
  	...
  	li $v0, 1		# System call code for print_int
  	move $a0, $t6		# Puts the result of multiplication into the register for the first argument
  	syscall			# Prints the integer based on system call code
  	...
  	add $t5, $t5, $t6	# Adds multiplication of the two integers to the total count
  	addi $t6, $zero, 1	# Multiplied numbers = 1 again
...
exit:
	li $v0, 4		# System call code for print_string
  	la $a0, final		# Puts the equal sign into the register for the first argument
  	syscall			# Prints the string based on system call code
...
###################################
readNum:
	li $t7, 0 		# Return value = 0 initially
	li $t3, 1		# Sign = 1
loop2:	
	lb $t2, 0($a1)
...
continue:
	...
	mul $t7, $t7, 10	# Shifts current number to the left by one
	addi $t4, $t2, -48	# Subtracts ASCII value of current char by ACII value of char '0'
	add $t7, $t7, $t4	# Adds to the current number
...
incReturn:
	...
	mul $t7, $t7, $t3	# Multiplies the retrieved value by the sign
	jr $ra			# Returns back to the thread that called it
